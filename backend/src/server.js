...
import { initDB, db, verifyPaymentTables }   from "./config/db.js";
...
// Load environment variables from the parent directory (backend_api root)
...
async function startServer() {
 ...
 // Initialize User table and test configurations
 try {
   const userModel = new User(db);
   await userModel.createTable();
   console.log('✅ User model initialized successfully');
   
   // Verify payment system tables (they're already created in initDB)
   console.log('🔧 Verifying payment system tables...');
   const paymentTablesStatus = await verifyPaymentTables();
   const allTablesExist = Object.values(paymentTablesStatus).every(table => table.exists);
   
   if (allTablesExist) {
     console.log('✅ Payment system tables verified successfully');
     console.log('📊 Payment tables status:');
     Object.entries(paymentTablesStatus).forEach(([table, status]) => {
       console.log(`   ${table}: ${status.count} records`);
     });
   } else {
     console.log('⚠️  Some payment tables are missing:');
     Object.entries(paymentTablesStatus).forEach(([table, status]) => {
       if (!status.exists) {
 ...
 app.get("/api/health", async (req, res) => {
   try { 
     ...
     try {
       paymentTablesStatus = await verifyPaymentTables();
     } catch (error) {
       console.log('Payment tables check failed:', error.message);
     }
     
     const allPaymentTablesOk = Object.values(paymentTablesStatus).every(table => table.exists);
     ...
     res.json({ 
       ...
       payment_tables_ok: allPaymentTablesOk,
       payment_tables_detail: paymentTablesStatus,
 ...
 app.get("/api/test-payments", async (req, res) => {
   try {
     ...
     // Test database tables using the new helper function
     const tablesStatus = await verifyPaymentTables();
